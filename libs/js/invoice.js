...
function calculateRow(row) {
        ...
        var indprice = row.find('input[name^="unit_cost"]').val();
        var indqty = row.find('input[name^="quantity"]').val();
	var total = 0;
	total = (price * qty);
        var rowtotal = (indprice * indqty);
         var discper = +row.find('input[name^="discount"]').val();
         var tax = +row.find('input[name^="tax"]').val();
              console.log(" Whole tax "+tax);
          var rowtax = row.find('input[name^="tax"]').val();
          console.log(rowtax);
          var taxamount = 0;
          if( tax != 0 ){
            taxamount = calculatePercent(rowtotal, rowtax);
          }
            console.log(" Whole tax "+taxamount);
          
          var ttltaxamount = calculatePercent(total, tax);
         total = total + ttltaxamount;
         var totalafterdisc = total - calculatePercent(total,discper);
        row.find('span[class^="amount"]').html("INR "+roundNumber(totalafterdisc,2));
        row.find('span[class^="price"]').html("INR "+roundNumber(rowtotal,2));
        row.find('input[class^="price-row"]').val(roundNumber(rowtotal,2));
        row.find('input[class^="tax"]').val(roundNumber(totalafterdisc,2));
        row.find('input[class^="taxamount"]').val(roundNumber(taxamount,2));
        row.find('input[class^="total-amount"]').val(roundNumber(totalafterdisc,2));
...
function calculateGrandTotal() {
	...
	$('.vat_rs').html("INR"+vat_rupees);
        ...
        $('.freight_rs').html("INR"+freight_rupees);
        $('#freight').val(freight_rupees);
	
	//$('.freight_rs').html("INR"+freight);
//    $('#freight_rupees').val(freight);
	//Grand total
	var final_total_amount = parseFloat(subTotal)+parseFloat(excise_rupees)+parseFloat(vat_rupees)+parseFloat(freight_rupees);
...
$(document).ready(function() {
   ...
   $("#addrow").click(function(){
	...
	htm+='<tr class="item-row">';
		htm+='<td class="item-name">';
			...
			htm+='</div>';
		htm+='</td>';
			htm+='<td class="services">';
			htm+='<textarea name="item_services[]" class="item-services"></textarea>';
		htm+='</td>';
		...
		htm+='</td>';
                htm+='<td>';
			htm+='<span class="price">0.0</span>';
                        htm+='<input type="hidden" class="price-row" name="price_row[]"/>';
		htm+='</td>';
                htm+='<td>';
			htm+='<input type="text" name="discount[]" value="0" class="price allownumeric" placeholder="Discount"/>';
		htm+='</td>';
                htm+='<td>';
			htm+='<input type="text" name="tax[]" value="0" class="price allownumeric" placeholder="Tax"/><input type="hidden" name="taxamount[]" value="0" class="taxamount allownumeric"  />';
		htm+='</td>';                
		htm+='<td>';
  ...
  $("table#items").on("change", 'input[name^="quantity"], input[name^="discount"] ,input[name^="unit_cost"],input[name^="tax"]', function (event) {
...
//		}
		...
		if(($.trim($("#our_company").val())).length==0) {
			errors.push("Customer name should not be empty");
		}
		if(($.trim($("#our_address").val())).length==0) {
			errors.push("Customer address should not be empty");
		}
		...
		if(($.trim($("#billing_address").val())).length==0) {
			errors.push("Customer address name should not be empty");
		}
		if(($.trim($("#billing_city").val())).length==0) {
			errors.push("Customer city name should not be empty");
		}
...
//		}
		if(($.trim($("#shipping_company").val())).length==0) {
			errors.push("Vehicle Year should not be empty");
		}
		if(($.trim($("#shipping_addr").val())).length==0) {
			errors.push("Vehicle model should not be empty");
		}
		if(($.trim($("#shipping_city").val())).length==0) {
			errors.push("Vehicle color should not be empty");
		}
