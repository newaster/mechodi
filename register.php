...
require_once('/home/embarkrms.com/public_html/mail_function.php');
$message = "";
if (isset($_POST['submit'])) {
    ...
    $countryCode = ( isset($_POST['countryCode']) && "" != $_POST['countryCode'] ) ? $_POST['countryCode'] : "";
    $contact = ( isset($_POST['contact']) && "" != $_POST['contact'] ) ? $_POST['contact'] : "";
    $city = ( isset($_POST['city']) && "" != $_POST['city'] ) ? $_POST['city'] : "";

    $selectRec = "Select CASE WHEN email = '" . $email . "' THEN 'Email' WHEN uname = '" . $username . "' THEN  'Username' END as result from login where email = '" . $email . "' or uname = '" . $username . "' ";
    $arrUser = find_by_sql($selectRec);
    if (sizeof($arrUser) > 0) {

        $message = $arrUser[0]['result'] . " already exists!";
    } else {
        $generatedPwd = time();
        $password = sha1($generatedPwd);
        $insertRec = "Insert into login ( uname, fname, lname, email, company, country_code,number,password,cleartext_pass,city,role,created_date ) values  ";
        $insertRec .= " ( '{$username}','{$first_name}' , '{$last_name}' ,"
                . " '{$email}' , '{$company}', '{$countryCode}', '{$contact}', "
                . "'{$password}' , '{$generatedPwd}' , '{$city}' , 'user', NOW() ) ";

        $db->query($insertRec);
        if ($db->insert_id() > 0) {
            $mailbody = "Please Login to Embarkrms with following credentials <br/>";
            $mailbody .= "Username : " . $username . "<br/>";
            $mailbody .= "Email : " . $email . "<br/>";
            $mailbody .= "Password : " . $generatedPwd . "<br/>";

            $emailheaders = "MIME-Version: 1.0" . "\r\n";
            $emailheaders .= "Content-type:text/html;charset=ISO-8859-1" . "\r\n";
            $emailheaders .= "From: admin@embarkrms.com" . "\r\n";

            cmail($email, "Embark MBR Login Details", $mailbody, $emailheaders);
            $message = "Registration Successfull! Please proceed with login.";
        }
    }
}
?>
...
<html class="no-js " lang="en">
    <head>
        ...
        <title>:: Embark Admin :: Sign Up</title>
        <!-- Favicon-->
                ...
                <div class="row">
                    <div class="col-lg-4 col-sm-12">
                        <form method="post" class="card auth_form"   >
                            <div class="header">
                                <img class="logo" src="assets/images/Logo-1.png" alt="">
                                <h5>Sign Up</h5>
                            ...
                            <div class="body">
                                ...
                                <div class="form-group label-floating">
                                    ...
                                    <select name="countryCode" required="required" id="" class="form-control">
                                ...
                                <button type="submit"  name="submit" class="btn btn-primary btn-block waves-effect waves-light">SIGN UP</button>
