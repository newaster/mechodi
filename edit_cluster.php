...
if ($id != 0) {
     $selectClu = "Select * from cluster where id = '$id' and updated_by = '$userid' ";
    $cluster = find_by_sql($selectClu);

    if (sizeof($cluster) < 1) {
        echo "<script>alert('Cluster not found!!!');</script>";
        ...
        $arrSelectCummulativeHeaders = explode(",", $cluster['cumulativeheaders']);
        $arrSelectNumbers = array_values(explode(",", $cluster['numbers']));
...
if (isset($_REQUEST['name'])) {
    ...
    $selectClu = "Select * from cluster where name = '$cluster_name' and updated_by = '$userid' ";

    $cluster = find_by_sql($selectClu);
    if (sizeof($cluster) > 0) {
        $insertRec = "UPDATE cluster set `name` = '$cluster_name' , `numbers` = '$sites'   , `headers` =  '$headers' , `cumulativeheaders` = '$cummulative'  ";
        $insertRec .= " WHERE  ";
        $insertRec .= "  id = '$id' ";
...
$selectRec = "Select embark_data.COLNAME from embark_data INNER JOIN sites on sites.number = embark_data.number where   LENGTH(COLNAME) > 35 and sites.status = 'active'  and  sites.created_by = '{$_SESSION['user_id']}' order by embark_data.id asc ";
    ...
    <div class="container-fluid">
        ...
        <div class="row clearfix">
            <div class="col-md-12 col-lg-12">
                <div class="card">
                    <div class="header">
                        <h2><strong>Edit</strong> Cluster</h2>
                    ...
                    <div class="body">
                        <form id="cluster_form"  action="edit_cluster.php?id=<?=$id;?>" name="cluster_form" method="post"   >
                            <div id="wizard_horizontal">

                                <h2>Enter Cluster Name</h2>
                                <section  style="width:100%;height: 100%;">

                                    <center> <h2>Enter Cluster Name</h2><input required="required" class="form-control col-3 form-widget" type="text" name="name" value="<?php echo $select_clu_name;?>" id="cluster_name"  ></center>
                                ...
                                <section  style="width:100%">
                                    ...
                                    <select id="select_sites"  required class="ms form-widget" multiple="multiple"   >
                                        ...
                                        <option <?php echo  ( true === in_array($valSite['number'], $arrSelectNumbers) ) ? "selected='selected'" : ""; ?>   value="<?php echo $valSite['number']; ?>"><?php echo $valSite['name']; ?></option>
                                        <?php } ?> 
                                        ...
                                        <div class="col-lg-6 col-md-6">
                                            ...
                                            <select id="headers"  required class="form-control show-tick ms select2 form-widget" multiple data-placeholder="Select" >
                                                <?php foreach ($arrMBR as $keyMBR => $valMBR) { ?>
                                                    <option <?php echo  ( true === in_array($valMBR, $arrSelectHeaders) ) ? "selected='selected'" : ""; ?>    value="<?php echo $valMBR; ?>"><?php echo strtoupper($valMBR); ?></option>
                                                <?php } ?> 
                                            </select>
                                        </div>
                                    </center>
                                    ...
                                    <center>
                                        <div class="col-lg-6 col-md-6">
                                            ...
                                            <select  id="cummulative"   required class="form-control show-tick ms select3 form-widget" multiple data-placeholder="Select" >
                                                <?php foreach ($arrMBR as $keyMBR => $valMBR) { ?>
                                                    <option <?php echo  ( true === in_array($valMBR, $arrSelectCummulativeHeaders) ) ? "selected='selected'" : ""; ?>  value="<?php echo $valMBR; ?>"><?php echo strtoupper($valMBR); ?></option>
                                                <?php } ?> 
                                            </select>
                                        </div>
                                    </center>
